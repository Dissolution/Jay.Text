...
#region Fluent CS File
        ...
        return codeBuilder
            ...
            .IndentBlock("// ",
                ib =>
                {
                    ...
                    {
                        ib.AppendLine(comment.AsSpan(start, length));
                    }
                })
            .AppendLine("// </auto-generated>");
    }
    ...
    public static CodeBuilder Using(this CodeBuilder codeBuilder, string nameSpace)
    {
        ...
        return codeBuilder.Append("using ").Append(ns).Append(';').NewLine();
    }
    ...
    public static CodeBuilder Namespace(this CodeBuilder codeBuilder, string? nameSpace)
    {
        ...
        {
            return codeBuilder.Append("namespace ").Append(nameSpace).AppendLine(';');
        }
    ...
    public static CodeBuilder Comment(this CodeBuilder codeBuilder, string? comment)
    {
        ...
        switch (lines.Count)
        {
            ...
            case 1:
                // Single line
                return codeBuilder.Append("// ").AppendLine(comment);
            default:
            {
                using var e = lines.GetEnumerator();
                e.MoveNext();
                codeBuilder.Append("/* ").AppendLine(comment.AsSpan(e.Current.start, e.Current.length));
                while (e.MoveNext())
                {
                    codeBuilder.Append(" * ").AppendLine(comment.AsSpan(e.Current.start, e.Current.length));
                }

                return codeBuilder.AppendLine(" #1#");
            }
        }
    }
    ...
    public static CodeBuilder Comment(this CodeBuilder codeBuilder, string? comment, CommentType commentType)
    {
        ...
        if (commentType == CommentType.SingleLine)
        {
            foreach (var (start, length) in lines)
            {
                codeBuilder.Append("// ").AppendLine(comment.AsSpan(start, length));
            }
        }
